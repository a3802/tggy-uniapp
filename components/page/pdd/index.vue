<template>
  ...
  <view class="diy-goods" :style="{ background: itemStyle.background }">
	<view class="title">
		<view class="title-icon"><image class="image" src="/static/hotgoods.png"></image></view>
        ...
        <view class="goods-item" v-for="(dataItem, index) in dataList" :key="index" @click="onTargetGoods(dataItem.mobile_url)">
          ...
          <block v-if="itemStyle.column === 1">
            <view class="dis-flex">
              ...
              <view class="goods-item_left">
                <image class="image" :src="dataItem.goods_thumbnail_url"></image>
              </view>
              <view class="goods-item_right">
                ...
                <view class="goods-item_desc">
                  ...
                  <view class="desc_footer">
                    <text v-if="itemStyle.show.includes('goodsPrice')" class="price_x">¥{{(dataItem.min_group_price - dataItem.coupon_discount)/100}}</text>
                    <text class="price_y col-9"
                      v-if="itemStyle.show.includes('linePrice') && dataItem.line_price_min > 0">¥{{ dataItem.min_normal_price/100 }}</text>
                  </view>
                </view>
              </view>
            </view>
          ...
          <block v-else>
            ...
            <view class="goods-image">
              <image class="image" mode="aspectFill" :src="dataItem.goods_thumbnail_url"></image>
            </view>
            <view class="detail">
              <!-- 商品标题 -->
              <view v-if="itemStyle.show.includes('goodsName')" class="goods-name">
              ...
              <view class="detail-price oneline-hide">
                <text v-if="itemStyle.show.includes('goodsPrice')" class="goods-price f-30 col-m">￥{{(dataItem.min_group_price - dataItem.coupon_discount)/100}}</text>
                <text v-if="itemStyle.show.includes('linePrice') && dataItem.line_price_min > 0"
                  class="line-price col-9 f-24">￥{{ dataItem.min_normal_price/100 }}</text>
              </view>
            </view>
          </block>
